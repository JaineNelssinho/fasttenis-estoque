...
const LOGO = 'https://i.imgur.com/ZR3UR20.png';
...
export default function App() {
  ...
  const loadData = useCallback(async (silent = false) => {
    try {
      const res = await fetch(`${BURL}/latest`, { headers: H });
  ...
  const save = async (np: any[], nh: any[]) => {
    setSaving(true);
    try {
      await fetch(BURL, {
        method: 'PUT',
        headers: H,
        body: JSON.stringify({ products: np, history: nh }),
      });
      setSyncTime(new Date().toLocaleTimeString('pt-BR'));
    } catch {
  ...
  const commit = (np: any[], nh: any[]) => {
    setProducts(np);
    setHistory(nh);
    save(np, nh);
  };
  const mkEntry = (h: any[], e: any) => [
    { ...e, id: Date.now(), date: new Date().toLocaleString('pt-BR') },
    ...h,
  ];
  ...
  const registerMove = () => {
    ...
    commit(
      products.map((p) =>
        p.id === prod.id
          ? {
              ...p,
              qty: moveForm.type === 'entrada' ? p.qty + qty : p.qty - qty,
            }
          : p
      ),
      mkEntry(history, {
        ...
        qty,
        note: moveForm.note || '-',
        responsavel: moveForm.responsavel || 'Não informado',
      })
    );
    setMoveForm({
      productId: '',
    ...
    showToast(
      `${moveForm.type === 'entrada' ? 'Entrada' : 'Saída'} registrada!`
    );
  };
  ...
  if (loading)
    return (
      ...
      >
        ...
        />
        <div style={{ color: '#666' }}>Conectando...</div>
      </div>
    );
  ...
  return (
    ...
    >
      <style>{`*{box-sizing:border-box;margin:0;padding:0}input,select{background:#1e1e2a;border:1px solid #2e2e42;color:#e8e6f0;border-radius:8px;padding:10px 14px;font-size:14px;outline:none;width:100%;margin-bottom:10px}button{cursor:pointer}.card{background:#16161f;border:1px solid #22222e;border-radius:14px;padding:16px;margin-bottom:12px}.tag{display:inline-block;padding:3px 10px;border-radius:20px;font-size:12px;font-weight:600}.row{display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px}.mini{text-align:center;background:#1a1a24;border-radius:8px;padding:8px}`}</style>
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {syncTime && !saving && (
              <span style={{ fontSize: 11, color: '#444' }}>✓ {syncTime}</span>
            )}
            {alerts.length > 0 && (
              <div
                style={{
          ...
          {[
            ...
            <button
              ...
              style={{
                background: 'none',
                border: 'none',
                color: view === k ? '#6c63ff' : '#666',
                padding: '13px 12px',
                fontSize: 13,
                fontWeight: 600,
                whiteSpace: 'nowrap',
                borderBottom:
                  view === k ? '2px solid #6c63ff' : '2px solid transparent',
              }}
            >
              {l}
            </button>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '20px 16px' }}>
        {view === 'dashboard' && (
          <div>
            ...
            >
              {[
                ['🎾', products.length, 'Produtos', '#6c63ff'],
                ['💰', `R$${totalValue.toFixed(0)}`, 'Valor', '#27ae60'],
                ['🔄', history.length, 'Movimentações', '#3498db'],
                [
                  '⚠️',
                  alerts.length,
                  'Alertas',
                  alerts.length > 0 ? '#e74c3c' : '#444',
                ],
            ...
            {alerts.length > 0 && (
              <div
                className="card"
                style={{ border: '1px solid #c0392b33', background: '#160a0a' }}
              >
                <div
                  style={{
                    fontSize: 13,
                  ...
                  >
                    <div>
                      ...
                      {h.responsavel && h.responsavel !== '-' && (
                        <div style={{ color: '#9b8fff', fontSize: 11 }}>
                          👤 {h.responsavel}
                        </div>
                      )}
                    </div>
            ...
            >
              {CATEGORIES.map((c) => (
                <button
                  ...
                  style={{
                    background: filterCat === c ? '#6c63ff' : '#1e1e2a',
              ...
              <div key={p.id} className="card">
                <div className="row">
                  <div>
                    ...
                    <span
                      className="tag"
                      style={{ background: '#1e1e2e', color: '#9b8fff' }}
                    >
                      {p.category}
                    </span>
                  </div>
                ...
                >
                  ...
                  <div className="mini">
                    <div
                      style={{ fontSize: 17, fontWeight: 700, color: '#666' }}
                    >
                      {p.minQty}
                    </div>
          ...
          <div>
            ...
            <div className="card">
                      ...
                      background:
                        moveForm.type === t
                          ? t === 'entrada'
                            ...
                            : '#c0392b'
                          : 'transparent',
                      color: moveForm.type === t ? 'white' : '#666',
                      fontWeight: 700,
                      fontSize: 14,
                    }}
                  >
              ...
              <input
                ...
                value={moveForm.qty}
                onChange={(e) =>
                  setMoveForm((f) => ({ ...f, qty: e.target.value }))
                }
              />
              <input
                ...
                onChange={(e) =>
                  setMoveForm((f) => ({ ...f, responsavel: e.target.value }))
                }
              />
              <input
        ...
        {view === 'historico' && (
          <div>
            ...
            {history.length > 0 ? (
                ...
                <div key={h.id} className="card">
                  <div className="row">
                    <div>
                      ...
                      <div
                        style={{ color: '#444', fontSize: 11, marginTop: 2 }}
                      >
                        {h.date}
                      </div>
                    </div>
              ...
              <div
                className="card"
                style={{ textAlign: 'center', padding: 48, color: '#555' }}
              >
